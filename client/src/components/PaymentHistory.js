...
const PaymentHistory = () => {
  ...
  const fetchPaymentsAndRefunds = async () => {
    try {
      ...
      console.log('user._id from localStorage:', user?._id);

      if (!user || !token) {
        console.error('User not authenticated - this should be handled by the route');
      ...
      console.log('Payments API URL:', requestUrl);
      ...
      // Fetch payments
      const paymentResponse = await axios.get(requestUrl, { headers });
      ...
      console.error('Error fetching payments and refunds:', {
      ...
      if (error.response?.status === 401) {
        ...
        toast.error('Invalid request. Check user ID or date format.');
  ...
  const checkNotifications = async () => {
    ...
    // First, check for refund updates
    try {
      const refundResponse = await axios.get(
        `${API_BASE_URL}/api/refunds/notifications?lastChecked=${lastChecked}&userId=${user._id}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
      ...
      if (refundResponse.data && refundResponse.data.length > 0) {
        // Update payments with new refund status
        setPayments(prevPayments =>
          prevPayments.map(payment => {
            const notification = refundResponse.data.find(n => n.paymentId === payment._id);
            if (notification) {
              return {
                ...payment,
                refundStatus: notification.status,
                refundDecisionDate: notification.decisionDate,
      ...
      console.log('Refund notifications check failed, continuing without refund updates:', refundError.message);
    }

    // Then check for general notifications
    try {
      ...
      // Process general notifications if needed
      console.log('General notifications:', notificationsResponse.data);
    } catch (notifError) {
      console.log('General notifications check failed, continuing without updates:', notifError.message);
    }

    // Update last checked time after both requests
  ...
  const handleRequestRefund = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/api/refunds/request`, {
