...
const API_BASE_URL = 'http://localhost:5000/api/refunds';
...
const AdminRefundDashboard = () => {
  ...
  const fetchRefunds = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/admin?${query.toString()}`, {
        headers: {
                  ...
                  {filteredRefunds.length === 0 ? (
                    ...
                    filteredRefunds.map(refund => (
                      <tr key={refund._id} style={{ transition: 'background-color 0.2s', verticalAlign: 'middle' }}>
                        ...
                        <td style={{ padding: '8px', fontSize: '0.95rem', textAlign: 'center' }}>${refund.amount.toFixed(2)}</td>
